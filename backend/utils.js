...
import geneds from "./catalog-files/geneds.js";
import csDegree from "./catalog-files/csDegree.js";
import mainHonors from "./catalog-files/mainHonors.js";

const COURSENUMBER =
    "CourseNumberTitleCRTypeGradeRptHrsErnHrsGpaQualPtsGPAHrsAtt",
  PAGELINE = "Page",
...
function cleanUp(lines) {
  ...
  while (lines[i] !== DEGREEINFORMATION) {
    if (
      foundTermTotal &&
      lines[i].slice(9, 9 + LenAcademicYear) === ACADEMICYEAR
    ) {
...
async function getJsonFromPdf(filePath, resultFileDir) {
...
function generateSingleReport(transcript) {
  ...
  // combining all geneds and cs courses to course name
  var allCourses = new Map();
  geneds.forEach((course) => {
    allCourses.set(course.courseId, course.courseName);
  });
  ...
  geneds.forEach((course) => {
    if (studentCourses.has(course.courseId)) {
      taken.push({
        ...
        courseGrade: studentCourses.get(course.courseId),
      });
    } else if (honorsCourses.has(course.honorsEqual)) {
      // this course has a honors equal
      honors.push({
        courseName: course.courseName,
        courseId: course.honorsEqual,
        courseGrade: studentCourses.get(course.honorsEqual),
      });
    } else {
      notTaken.push({
        courseName: course.courseName,
